...
from f_finder._classes import f_finder
...
def test_url(urls, expected_urls):
    assert (
        f_finder.check_url(urls, headers=None, proxies=None) == expected_urls
    )  # noqa: E501
...
def test_proxy(proxies, expected_proxies):
    assert f_finder.get_proxy(proxies) == expected_proxies
